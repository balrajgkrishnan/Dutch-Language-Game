...
export const AnimalAvatar: React.FC<AnimalAvatarProps> = ({
  ...
  useEffect(() => setImageFailed(false), [animalId]);
  ...
  const useIllustration = !!matchedAnimal && !imageFailed;
  ...
  return (
    ...
    >
      {useIllustration ? (
        <motion.div
          animate={isAnimated ? { y: [0, -3, 0] } : {}}
          transition={{ repeat: Infinity, duration: 3, ease: 'easeInOut' }}
          className="relative w-full h-full rounded-2xl overflow-hidden"
        >
          <img
            src={`/animal-images/${matchedAnimal!.id}.png`}
            alt={matchedAnimal?.name || animalId}
